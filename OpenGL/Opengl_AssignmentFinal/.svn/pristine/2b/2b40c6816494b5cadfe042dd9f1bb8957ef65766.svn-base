...
bool CreateMainWindow( const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState )
{
	...
	{
		s_mainWindow = CreateMainWindowHandle( i_thisInstanceOfTheProgram, i_initialWindowDisplayState, 800, 600 );
		//Initialize(s_mainWindow);
...
bool CreateMainWindowWIthSize( const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState,const int w_width,const int w_height )
{
	...
	if ( mainWindowClass != NULL )
	{
		s_mainWindow = CreateMainWindowHandle( i_thisInstanceOfTheProgram, i_initialWindowDisplayState,w_width,w_height  );
		//Initialize(s_mainWindow);
		if ( s_mainWindow == NULL )
		{
			goto OnError;
...
//bool ( const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState, int w_width, int w_height)
...
HWND CreateMainWindowHandle( const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState,const int desiredWidth,const int desiredHeight )
{
	...
	HWND mainWindow;
	{
		...
		const char* windowCaption = "Swapp's EAE6320 Game";
		// The window's style
...
ATOM RegisterMainWindowClass( const HINSTANCE i_thisInstanceOfTheProgram )
{
	...
	wndClassEx.hIcon = LoadIcon( i_thisInstanceOfTheProgram, MAKEINTRESOURCE( IDI_WINDOWSPROGRAM ) );
	...
	case WM_CHAR:
		{
			...
			if ( i_wParam == VK_ESCAPE )
			{
				...
				int result;
				{
					...
					if (BlackRock::UserSettings::IsFullScreenModeEnabled())
					{
						result = IDYES;
					}
					else
