...
local function BuildAsset( i_relativePath, i_extension, i_builderFileName, i_argument )
	...
	-- You don't need to understand this, but if you're curious read on.
	...
	local path_target = s_BuiltAssetDir .. i_extension
		...
		do
			...
			do
				if type( i_builderFileName ) == "string" then
					local builderPath = s_BinDir .. i_builderFileName
			...
			local builderTypeArg = i_argument
			local arguments = "\"" .. path_source .. "\" \"" .. path_target .. "\" \"" .. builderTypeArg .. "\""
...
local function BuildAssets( i_assetsToBuild )
	...
	for assetType, assetInfo in pairs( i_assetsToBuild ) do
		...
		--print(("BuilderName " .. builderFileName))
		...
		for i, assetToBuild in ipairs( assetsToBuild ) do
			--for j, assetName in ipairs(assetToBuild) do
				local assetNameP = assetToBuild.name
				local assetNameE = assetToBuild.extension
				local assetArgumentP = ""
				if assetToBuild.arguments ~= nil then
					for j, assetArgument in ipairs(assetToBuild.arguments) do
						assetArgumentP = assetArgument
					end
				end
				

				--local relativePath = assetToBuild
				--print("Asset :" .. assetNameP .. ", Builder :" .. builderFileName .. ",Argument :" .. assetArgumentP)
				if not BuildAsset( assetNameP, assetNameE, builderFileName, assetArgumentP ) then
...
if commandLineArgument then
	...
	if DoesFileExist( path_assetsToBuild ) then
		...
		--print("aasses " .. path_assetsToBuild)
